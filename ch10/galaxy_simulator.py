...
def detect_prob(disc_vol_scaled):
    ...
    if ratio < 0.002:   # set very low ratios to probability of 0
        ...
        detection_prob = -0.00474 * ratio**4 + 0.06661 * ratio**3 - 0.3597 * \
                        ratio**2 + 0.92 * ratio + 0.009193
...
def main():
    ...
    c.create_text(
        -455, -330, fill='white', anchor='w',
        text=f'Radio Bubble Diameter = {SCALE} LY'
        )
    c.create_text(
        -455, -300, fill='white', anchor='w',
        text=f'Probability of detection for {NUM_CIVS:,} civilizations = '
        f'{detection_prob}'
        )
