...
def prep_words(name, word_list_ini):
    ...
    len_name = len(name)
    word_list = [
            word.lower() for word in word_list_ini if len(word) == len_name
            ]
...
def letter_pair_filter(filter_2):
    ...
    filtered = set()
    rejects = [
        'dt', 'lr', 'md', 'ml', 'mr', 'mt', 'mv', 'td', 'tv', 'vd', 'vl', 'vm',
        'vr', 'vt',
        ]
    first_pair_rejects = [
                        'ld', 'lm', 'lt', 'lv', 'rd', 'rl', 'rm', 'rt', 'rv',
                        'tl', 'tm',
                        ]
...
def view_by_letter(name, filter_3):
    ...
    first = input("select a strating letter or press Enter to see all: ")
    subset = []
