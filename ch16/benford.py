...
def count_first_digits(data_list):
    ...
    for sample in data_list:
        ...
        try:
            ...
            print(e)
            print("Sample must be integers. Exiting.")
            sys.exit(1)
...
def chi_square_test(data_count, expected_counts):
    """
    Return boolean on chi-square text (8 degrees of freedom & P-val=0.05).
    """
    chi_square_stat = 0     # chi-square test statistic
    for data, expected in zip(data_count, expected_counts):
        chi_square = math.pow(data - expected, 2)
        chi_square_stat += chi_square / expected
    print(f"\nChi-square Test Statistic = {chi_square_stat:.3f}")
...
def bar_chart(data_pct):
    ...
    # text for labels, titles, and ticks
    ...
    for rect in rects:
        ...
        ax.text(
            rect.get_x() + rect.get_width()/2, height, f'{height:0.1f}',
    ...
    # Hide the right and top splines & add legend
...
def main():
    ...
    for i in range(1, 10):
        print(
            f"{i}: observed: {data_pct[i - 1] / 100:.3f}    "
