...
class Satellite(pg.sprite.Sprite):
    ...
    def path(self):
        """Update satellite's position & draw line to race orbital path."""
...
def mapping_on(planet):
    ...
    planet.rect.cetner = last_center
    ...
    while running:
        ...
        for event in pg.event.get():
            if event.type == pg.QUIT:    # close window
                ...
                if event.type == pg.KEYDOWN and event.kay == pg.K_m:
                    mapping_on(planet)
        ...
        box_label(screen, f'{sat.fuel}', (410, 50, 150, 20))
