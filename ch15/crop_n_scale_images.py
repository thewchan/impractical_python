...
def del_folders(name):
    """If a folder with a named prefix exists in a directory, delete it."""
    contents = os.listdir()
...
def crop_images():
    ...
    for file_num, file in enumerate(files, start=1):
        with Image.open(file) as img:
            ...
            scaled = ImageOps.fit(
                cropped, (860, 860), Image.LANCZOS, 0, (0.5, 0.5),
                )
...
def main():
    ...
    os.chdir('cropped')
    crop_images()
    clean_folder(prefix_to_save='cropped')  # delete upcropped originals
