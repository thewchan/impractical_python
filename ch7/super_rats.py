...
def select(population, to_retrain):
    """Cull a population to return only a specified number of members."""
    sorted_population = sorted(population)
    to_retain_by_sex = to_retrain//2
...
def breed(males, females, litter_size):
    """Crossover gene among members (weights) of a population."""
...
def mutate(children, mutate_odds, mutate_min, mutate_max):
    ...
    for index, rat in enumerate(children):
        if mutate_odds >= random.random():
            children[index] = round(rat * random.uniform(
...
def main():
    ...
    generations = 0
    parents = populate(
        NUM_RATS, INITIAL_MIN_WT, INITIAL_MAX_WT, INITIAL_MODE_WT,
        )
